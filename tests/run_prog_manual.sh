...
abs_top_builddir="/home/christoph/cob/gnucobol-3.1-rc1"
...
"
  ...
  screen -S "GCTESTS" -X exec ... bash -c "cd \"$PWD\" && ($COBCRUN_DIRECT $1 2>./syserr.log && echo $? > ./result) || echo 1 > ./result"
}
...
if test ! -z "$DISPLAY"; then
  _test_with_xterm  $1 || echo $? > ./result
else
  _test_with_screen $1 || echo $? > ./result
fi
